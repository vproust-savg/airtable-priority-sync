...
from sync.core.utils import clean, format_price, priority_yn, to_int
...
# Priority field names for allergens (for $select in P→A fetch)
ALLERGEN_PRIORITY_FIELDS: list[str] = [m.priority_field for m in ALLERGEN_FIELD_MAP]
...
# ─────────────────────────────────────────────────────────────────────────────
# P→A Reverse mapping functions (Priority → Airtable direction)
# ─────────────────────────────────────────────────────────────────────────────

def map_allergens_to_airtable(priority_fields: dict[str, Any]) -> dict[str, Any]:
    """
    Map Priority SAVR_ALLERGENS fields → Airtable allergen/feature field names.

    Uses the same ALLERGEN_FIELD_MAP in reverse:
    reads priority_field from Priority data, writes to airtable_field.
    Applies priority_yn transform for Y→Yes / N→No conversion.

    Args:
        priority_fields: Raw allergen fields from Priority API response.
    ...
    payload: dict[str, Any] = {}
    for mapping in ALLERGEN_FIELD_MAP:
        raw = priority_fields.get(mapping.priority_field)
        # All allergen fields use priority_yn for P→A (Y→Yes, N→No)
        value = priority_yn(raw)
        if value is not None:
            payload[mapping.airtable_field] = value
    return payload
