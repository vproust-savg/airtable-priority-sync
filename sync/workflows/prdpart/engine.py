"""
PRDPART sync engine: MRP Parameters for Parts.

Subclasses BaseSyncEngine with PRDPART-specific configuration.
No sub-forms — all fields live on the main PRDPART entity.
No STATUS mode — only 7 fields, always sync all.
"""
...
from sync.workflows.prdpart.config import (
    AIRTABLE_KEY_FIELD,
...
class PrdpartSyncEngine(BaseSyncEngine):
    """
    Sync engine for PRDPART (MRP Parameters for Parts).

    Simple engine with no sub-forms. Uses use_filter_lookup=True
    because PRDPART uses integer keys in URLs (same as FNCPART).
    """
    ...
    def _create_priority_client(self, api_url_override: str | None = None) -> PriorityClient:
        return PriorityClient(
            ...
            use_filter_lookup=True,  # PRDPART uses integer keys, not string PARTNAME
