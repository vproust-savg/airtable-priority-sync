...
from sync.core.models import SyncStats
...
class SyncLogClient:
    ...
    def __init__(self, entity_label: str = "SKU") -> None:
        """
        Args:
            entity_label: Label for the entity key in error records.
                         "SKU" for products, "Vendor" for vendors, etc.
        """
    ...
    def get_last_p2a_max_udate(self, workflow: str | None = None) -> str | None:
        """
        Read the Max UDATE from the last successful P→A sync run.

        Args:
            workflow: Optional workflow name to filter by (e.g., "products", "fncpart").
                     If None, matches any workflow.

        Returns:
            The UDATE string from the most recent successful P→A run,
            or None if no previous P→A run exists or logging is disabled.
        """
        if not self.enabled:
            return None
        ...
        try:
            ...
            filter_parts = ['{Direction}="P→A"', 'OR({Status}="success", {Status}="partial")']
            if workflow:
                filter_parts.append(f'{{Workflow}}="{workflow}"')
            ...
            if records:
                max_udate = records[0].get("fields", {}).get("Max UDATE")
                if max_udate:
                    logger.info("Last P→A Max UDATE: %s", max_udate)
                    return max_udate

            logger.info("No previous P→A run found — will fetch all records.")
            return None

        except Exception as e:
            logger.warning("Failed to read last P→A Max UDATE: %s", e)
            return None
    ...
    def log_run(
        ...
        Args:
            ...
            workflow: Workflow name (e.g., "products", "fncpart").
        ...
        try:
            run_record_id = self._write_run(stats, direction, trigger, workflow)
    ...
    def _write_run(
        self, stats: SyncStats, direction: str, trigger: str, workflow: str
    ) -> str | None:
        ...
        if stats.error_details:
            ...
            for err in stats.error_details[:20]:  # Cap at 20 lines
                ...
                lines.append(f"{err.entity_key}: {err.action} ({status_str}) — {err.message}")
            error_summary = "\n".join(lines)
